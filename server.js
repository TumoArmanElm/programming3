...
const Farmer = require("./classes/Farmer");
  ...
  for (let y = 0; y < matrix.length; y++) {
    ...
    for (let x = 0; x < matrix[y].length; x++) {
      if (matrix[y][x] == 1) {
        ...
        const newGrassEater = new GrassEater(
          x,
          y,
          2,
          matrix,
...
function statUpdater(creatureCount) {
  if (frameCount % 60 == 0){ 
    jsonStats = JSON.stringify(creatureCount);
    fs.writeFile("stats.json", jsonStats, function(err){
